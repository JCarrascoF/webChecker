...
class scrapper {
    ...
    async _getCacheContent() {
        ...
        try {
            ...
            console.log(`Not valid cache found. Creating new one.\n\n${error}\n\nStack:: ${error.stack}`);
    ...
    run() {
        return (async () => {
            ...
            try {
                ...
                if (newItems.length) {
                    mailContent = `New items!\n${JSON.stringify(newItems,0,1)}\n\nhttps://www3.emvs.es/smaweb/`;
                    await this._sendMail(mailContent);
                }
                await (this._updateCache(currentContent));
            } catch (error) {
                mailContent = `Error running EMVS scrapper\n\n${error}\n\nStacktrace:: ${error.stack}`;
